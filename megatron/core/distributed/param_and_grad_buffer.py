...
class Bucket:
    ...
    def __init__(
        ...
        self.param_to_index_map = param_index_map
        ...
        self.tp_world_size = parallel_state.get_tensor_model_parallel_world_size()
        self.pp_world_size = parallel_state.get_pipeline_model_parallel_world_size()

        self.tp_pp_size = self.tp_world_size * self.pp_world_size
        self.idx = -1
    ...
    def start_grad_sync(self, seeds):
        """
        ...
        # GMC+ communication
        if self.ddp_config.use_gmc_plus:
            with torch.no_grad():
            ...
            # Add weight decay to gradients if needed
                ...
                param_data = torch.empty_like(self.grad_data)
                for param in self.params:
                    ...
                    if hasattr(param, 'param_group_ref'):
                        p_lr = param.param_group_ref.get('lr', 0.001)
                        p_wd = param.param_group_ref.get('weight_decay', 0.0)
                    else:
                        p_lr = 0.001
                        p_wd = 0.0

                    lr_tensor[local_start:local_end] = p_lr
                    wd_tensor[local_start:local_end] = p_wd
                    param_data[local_start:local_end] = param.data.view(-1).to(self.grad_data.dtype)
                
                # considering weight decay
                self.grad_data += wd_tensor * param_data

                # first initialize
                if self.gmc_error is None:
                    self.gmc_error = torch.zeros_like(self.grad_data)
                    self.gmc_diff = torch.zeros_like(self.grad_data)
                # compute error_grads
                gmc_error_old = self.gmc_error.clone()
                self.gmc_error +=  self.grad_data - (self.gmc_beta / lr_tensor) * self.gmc_diff
                # sparse grads
                def seedk(grad_data, sparsity_rate, seed_):
                    dim = grad_data.shape[0]
                    random_size = int(sparsity_rate * dim)
                    np.random.seed(seed_)
                    samples_indices = torch.from_numpy(np.random.choice(np.arange(dim),size=(random_size,), replace=False)).to(self.grad_data.device)
                    mask = torch.zeros_like(grad_data, dtype = torch.bool).to(self.grad_data.device)
                    mask = mask.scatter(0, samples_indices, True)
                    return mask

                # set the seed according to the init_seed and the iteration number & acquire the mask and the sparse values for communication
                seed = seeds[self.idx]

                mask = seedk(self.gmc_error, self.gmc_sparse, seed)
                sparse_values = self.gmc_error[mask]

                # communication & update gmc_error
                ...
                self.grad_data[mask] = sparse_values
                self.grad_data += self.gmc_detached_coeff * (self.gmc_error - gmc_error_old)
    ...
    def finish_grad_sync(self, seeds):
        """
...
class ParamAndGradBuffer:
    ...
    def __init__(
        ...
        self.param_to_name = param_to_name  # Mapping from param to name 
        ...
        for param in params[::-1]:
            ...
            bucket_params.add(param)
            param_to_name_[param] = self.param_to_name[param]
