...
class DistributedDataParallel(MegatronModule):
    ...
    def __init__(
        ...
        for buffer in self.buffers + self.expert_parallel_buffers:
            buffer.set_idx(self.num_buckets)
            self.num_buckets += buffer.num_buckets
    ...
    def get_seed(self):
        """
        Returns the seed for the current iteration, which is used for GMC+ compression.
        """
        base_seed = self.cur_iter
        rng_group = np.random.RandomState(base_seed)
        tp_pp_seeds = rng_group.randint(0, 2**31 - 1, size=self.tp_pp_size) 
        group_idx = self.pp_rank * self.tp_world_size + self.tp_rank
        group_seed = tp_pp_seeds[group_idx]
